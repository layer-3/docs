...
Signatures in <Tooltip content={tooltipDefinitions.virtualAppProtocol}>VirtualApp</Tooltip> are stored as raw `bytes` so the protocol can validate multiple scheme formats. 
...
At a minimum VirtualApp currently recognizes the following signature families (see the [Signature Formats](./signature-formats) reference for the full specification):
