...
**Purpose**: Adjust <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> allocations by locking or unlocking funds **without closing the channel**.
...
**Use Cases**:
- Increasing allocations (positive delta: adding funds)
- Decreasing allocations (negative delta: removing funds)
...
| VOID | ACTIVE | `create()` with all sigs | All participants' signatures, sufficient balances |
