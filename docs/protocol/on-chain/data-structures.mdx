...
The legacy <Tooltip content={tooltipDefinitions.stateHash}>state hash</Tooltip> concept was removed in v0.3.0 when non-ECDSA signatures were introduced. Instead, participants sign the **packed state** payload directly:
