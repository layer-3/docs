...
- [ ] **Compute <Tooltip content={tooltipDefinitions.stateHash}>stateHash</Tooltip> from channel state**
...
- [ ] **Load contract ABIs**
  ...
  - <Tooltip content={tooltipDefinitions.adjudicator}>Adjudicator</Tooltip> contracts (SimpleConsensus, Remittance, etc.)
  - ERC-20 token ABI
...
- [ ] **Call Custody.join() if implementing clearnode role**
  ...
  - Verify <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> parameters match expectations
...
WebSocket-based communication with <Tooltip content={tooltipDefinitions.clearnode}>clearnode</Tooltip>.
...
- [ ] **Implement auth_request/challenge/verify flow**
  - Generate <Tooltip content={tooltipDefinitions.sessionKey}>session keypair</Tooltip> locally
...
- [ ] **Track session expiration**
  - Monitor session `expires_at` timestamp
