...
- VirtualApp stores the resulting `(v, r, s)` so adjudicators can re-create the prefixed hash for verification.
...
- During verification VirtualApp simulates or deploys the wallet, then forwards the inner signature to the regular EIP-1271 flow.
