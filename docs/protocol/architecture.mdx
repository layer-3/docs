...
graph TB
  ...
  APP --> CLIENT
  CLIENT <-->|"Communicate via WebSocket using NitroRPC protocol"| BROKER
...
```mermaid
graph TB
    A[User Wallet<br/>ERC-20] -->|deposit| B[Available<br/>Custody Contract]
    B -->|create/resize| C[Channel-Locked<br/>Custody SC]
    C -->|off-chain updates| D[Unified Balance<br/>Clearnode]
    D -->|open/deposit| E[App Sessions<br/>Application]
    E -->|withdraw / close session| D
    C -->|resize / challenge / close channel| B
    B -->|withdraw| A
...
1. **Deposit**: User deposits ERC-20 tokens into <Tooltip content={tooltipDefinitions.custodyContract}>Custody Contract</Tooltip>
2. **Lock**: Funds move from "available" to "locked" when <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> is created or resized
3. **Off-Chain**: Channel-locked funds become part of <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip>, managed off-chain by the clearnode
4. **App Sessions**: Funds can be allocated to <Tooltip content={tooltipDefinitions.appChannel}>app sessions</Tooltip> for applications
5. **Release**: When app sessions close or withdraw, funds return to unified balance
6. **Unlock**: Channel operations (resize, challenge, close) move funds from "locked" back to "available"
7. **Withdraw**: User withdraws available funds back to their wallet
