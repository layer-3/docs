...
2. The clearnode returns signed initial <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip> (signature at index 1)
3. Client signs the initial <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip> (signature at index 0)
4. Client submits transaction to <Tooltip content={tooltipDefinitions.blockchain}>blockchain</Tooltip> with both signatures
...
2. **Lock**: Funds move from "available" to "locked" when <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> is created/joined
3. **Off-Chain**: Locked funds become part of <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip>, managed off-chain
