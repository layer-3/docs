...
The client sends a registration request signed by their main <Tooltip content={tooltipDefinitions.walletAddress}>wallet address</Tooltip> (e.g., MetaMask):
...
- Check if wallet address has open <Tooltip content={tooltipDefinitions.channel}>channels</Tooltip>
...
The client signs the challenge with the <Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip> private key:
...
Open a payment <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> and deposit funds on blockchain.
...
- Creates initial <Tooltip content={tooltipDefinitions.channelState}>State</Tooltip> with `data` = `0x1EC7` (CHANOPEN magic number)
- Computes <Tooltip content={tooltipDefinitions.stateHash}>`stateHash`</Tooltip> = `keccak256(abi.encode(channelId, data, allocations))`
- Signs stateHash with clearnode's <Tooltip content={tooltipDefinitions.participant}>participant</Tooltip> key
...
- Signs stateHash with user's <Tooltip content={tooltipDefinitions.participant}>participant</Tooltip> key
...
- Transfers tokens from user to <Tooltip content={tooltipDefinitions.custodyContract}>Custody</Tooltip>
- Sets <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> status to `INITIAL`
...
<Tooltip content={tooltipDefinitions.clearnode}>The clearnode</Tooltip> submits transaction:
```javascript
...
- Transfers tokens from clearnode to <Tooltip content={tooltipDefinitions.custodyContract}>Custody</Tooltip> (0 in this case)
- Updates <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> metadata with actual deposits
...
<Tooltip content={tooltipDefinitions.clearnode}>The clearnode</Tooltip>:
- Updates internal database: <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> status = active
- Adds user's deposit to <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip>
- Sends `channel_update` notification to client
- Sends `balance_update` notification to client

#### Step 14: Channel Active

- User has 100 USDC in <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip>
...
- Client B exists (has open <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip>)
