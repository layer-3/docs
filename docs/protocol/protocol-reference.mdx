...
Constants used in the protocol to signal special <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip> types.
...
**Description**: Magic number in `state.data` for <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> funding states.
...
- Both <Tooltip content={tooltipDefinitions.participant}>participants</Tooltip> must sign state containing CHANOPEN
...
**Description**: Magic number in `state.data` for <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> closing states.
...
- All <Tooltip content={tooltipDefinitions.participant}>participants</Tooltip> must sign state containing CHANCLOSE
...
Smart contracts validate these magic numbers to ensure proper <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> lifecycle. Using incorrect magic numbers will cause transactions to revert.
:::
...
|------|----|---------|---------| 
| `VOID` | `INITIAL` | `create()` | <Tooltip content={tooltipDefinitions.creatorParticipant}>Creator</Tooltip> submits first |
| `INITIAL` | `ACTIVE` | `join()` | All <Tooltip content={tooltipDefinitions.participant}>participants</Tooltip> joined |
...
| `-32603` | Internal error | Internal server error | <Tooltip content={tooltipDefinitions.clearnode}>Clearnode</Tooltip> error |
...
| `1002` | Insufficient balance | User doesn't have enough available funds | Deposit more or close <Tooltip content={tooltipDefinitions.appChannel}>app sessions</Tooltip> |
| `1003` | Channel not found | Specified <Tooltip content={tooltipDefinitions.channelId}>channelId</Tooltip> does not exist | Verify <Tooltip content={tooltipDefinitions.channelId}>channelId</Tooltip>, create new <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> |
| `1004` | App session not found | Specified <Tooltip content={tooltipDefinitions.appSessionId}>appSessionId</Tooltip> does not exist | Verify ID, create new session |
...
| `1009` | Unsupported chain | Specified <Tooltip content={tooltipDefinitions.chainId}>chainId</Tooltip> not supported | Use `get_config` to see supported chains |
...
**Hash**: Raw <Tooltip content={tooltipDefinitions.stateHash}>stateHash</Tooltip> (no EIP-191 prefix for chain-agnostic compatibility)
...
**Example Path** (Polygon USDC <Tooltip content={tooltipDefinitions.custodyContract}>Custody</Tooltip>):
```
...
:::tip Asset Discovery
Use `get_assets` RPC method to retrieve the complete list of supported assets with contract addresses for each <Tooltip content={tooltipDefinitions.chainId}>chain</Tooltip>. Asset support may vary by clearnode.
:::
...
**Why not used**: Nitrolite signatures are computed over raw <Tooltip content={tooltipDefinitions.stateHash}>stateHash</Tooltip> without EIP-191 prefix to maintain compatibility across different EVM chains and potential non-EVM implementations.
