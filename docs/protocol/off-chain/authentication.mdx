...
```javascript
// 1. Generate session keypair (client-side, never transmitted)
const sessionKeyPair = generateKeyPair() // e.g., secp256k1
const sessionAddress = deriveAddress(sessionKeyPair.publicKey)

// 2. Create request parameters
const params = {
  address: userMainWalletAddress,
...
const req = [requestId, "auth_request", params, 0] // timestamp filled by server

// 4. User signs with main wallet (e.g., MetaMask prompt)
const signature = await userWallet.signMessage(keccak256(JSON.stringify(req)))

// 5. Send request
const response = await sendToC learnode({ req, sig: [signature] })

// 6. Extract challenge
const challenge = response.result.challenge_message
```
...
```javascript
// Continuing from auth_request example...

// 7. Sign challenge with session key (NOT main wallet)
const verifyParams = {
  challenge: challenge, // From auth_challenge response
  session_key: sessionAddress
}

const verifyReq = [requestId + 1, "auth_verify", verifyParams, 0]

// Sign with SESSION KEY private key
const challengeSignature = sign(keccak256(JSON.stringify(verifyReq)), sessionKeyPair.privateKey)

// 8. Send verification
const verifyResponse = await sendToClearnode({ req: verifyReq, sig: [challengeSignature] })

// 9. Authentication complete!
const { address, session_key, expire } = verifyResponse.result

// Store session info
storeSession({
  mainAddress: address,
  sessionKey: session_key,
  sessionPrivateKey: sessionKeyPair.privateKey, // Store securely!
  expiresAt: expire
})

// 10. Use session key for all subsequent requests
// Example:
const channelReq = [requestId + 2, "create_channel", {...}, 0]
const channelSig = sign(keccak256(JSON.stringify(channelReq)), sessionKeyPair.privateKey)
await sendToClearnode({ req: channelReq, sig: [channelSig] })
```
...
Use `get_session_keys` to view active sessions and their remaining allowances:

```javascript
const sessions = await clearnode.call("get_session_keys", {})

sessions.forEach(session => {
  console.log(`Session: ${session.session_key}`)
  console.log(`Expires: ${new Date(session.expire * 1000)}`)
  console.log(`Allowances:`)
  session.allowances.forEach(allowance => {
    const remaining = allowance.allowance - allowance.used
    console.log(`  ${allowance.asset}: ${remaining} of ${allowance.allowance} remaining`)
  })
})
```
...
After transfer of 45 USDC:
  allowance = 100 USDC
  used = 45 USDC
  remaining = 55 USDC

If next operation requires 60 USDC:
  Error: "Session key allowance exceeded: 60 required, 55 available"
```
