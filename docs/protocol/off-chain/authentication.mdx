...
2. Client controls the <Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip> private key (Step 3 signature)
...
Always specify `allowances` when creating <Tooltip content={tooltipDefinitions.sessionKey}>session keys</Tooltip>. If a <Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip> is compromised, allowances limit the maximum damage to the specified amounts.
:::
...
Request **MUST** be signed by the main <Tooltip content={tooltipDefinitions.participant}>wallet address</Tooltip> to prove ownership and authorize the <Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip>.
...
Client must call `auth_verify` with the challenge signed by the <Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip>.
...
Response is signed by <Tooltip content={tooltipDefinitions.clearnode}>the clearnode</Tooltip>.

**Verification**:
- Client SHOULD verify <Tooltip content={tooltipDefinitions.clearnode}>the clearnode's</Tooltip> signature on the response
...
Request **MUST** be signed by the <Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip> (NOT the main wallet) to prove ownership.
...
Session is authenticated. All subsequent private method calls should be signed with the <Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip>.
...
When a session expires, <Tooltip content={tooltipDefinitions.clearnode}>the clearnode</Tooltip> will reject requests with error code `1001`:
