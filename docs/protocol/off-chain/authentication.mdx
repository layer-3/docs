...
Authentication with <Tooltip content="A virtual ledger layer that provides a unified ledger (through Nitro RPC) and coordinates state channels (through Nitrolite), providing chain abstraction for developers and users">Clearnode</Tooltip> can be done in two ways: using your **main wallet as a root signer** for all requests, or delegating to <Tooltip content="A temporary cryptographic key delegated by a user's main wallet that provides a flexible way for the user to manage security of their funds by giving specific permissions and allowances for specific apps">session keys</Tooltip> via a secure 3-step challenge-response protocol.
...
2. **Session Keys (Delegated)**: Establish an authenticated session once, then use a <Tooltip content="A temporary cryptographic key delegated by a user's main wallet that provides a flexible way for the user to manage security of their funds by giving specific permissions and allowances for specific apps">session key</Tooltip> for subsequent operations without repeatedly prompting the main wallet.
...
sequenceDiagram
    ...
    Clearnode->>Client: Session established (address, expires_at)
...
| `expires_at` | number | No | Unix timestamp when the session key expires<br/><br/>**Format**: Unix timestamp (seconds since epoch) | 1 hour from authentication | `1762417328` | Absolute timestamp, not relative duration |
...
Automatically sent by <Tooltip content="A virtual ledger layer that provides a unified ledger (through Nitro RPC) and coordinates state channels (through Nitrolite), providing chain abstraction for developers and users">Clearnode</Tooltip> in response to valid `auth_request`. Client does not explicitly call this; it's part of the authentication flow.
...
| `expires_at` | number | Session expiration Unix timestamp | `1699999999` | After this time, must re-authenticate |
