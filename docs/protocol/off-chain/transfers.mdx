...
| `allocations` | Allocation[] | Yes (minimum: 1) | Assets and amounts to transfer<br/><br/>**Structure (per allocation)**:<br/>• `asset` (string, required) - Asset identifier<br/>&nbsp;&nbsp;- Examples: "usdc", "eth", "weth", "btc"<br/>&nbsp;&nbsp;- Note: Must be lowercase; see get_assets for supported assets<br/>• `amount` (string, required) - Amount to transfer in human-readable format<br/>&nbsp;&nbsp;- Format: Decimal string (e.g., "50.0" for 50 USDC)<br/>&nbsp;&nbsp;- Note: Clearnode handles conversion to smallest unit internally | See structure | `[{"asset": "usdc", "amount": "50.0"}, {"asset": "eth", "amount": "0.01"}]` | Can send multiple assets in single transfer<br/><br/>:::note Allocation Structure<br/>For detailed allocation structure definitions, see [Data Structures](../on-chain/data-structures#allocation-structure).<br/>::: |
...
|-------|------|-------------|---------|
| `id` | string | Unique transaction reference | `"1"` |
| `tx_type` | string | Transaction type | `"transfer"` |
| `from_account` | string | Sender's wallet address | `"0x1234567890abcdef..."` |
| `from_account_tag` | string | Sender's user tag (if exists) | `"NQKO7C"` |
| `to_account` | string | Recipient's wallet address | `"0x9876543210abcdef..."` |
| `to_account_tag` | string | Recipient's user tag (if exists) | `"UX123D"` |
| `asset` | string | Asset symbol that was transferred | `"usdc"` |
| `amount` | string | Amount transferred for this asset | `"50.0"` |
...
sequenceDiagram
    ...
    Clearnode->>Alice: {ledger_transactions}
    Clearnode->>Alice: balance_update notification
    Clearnode->>Bob: tr (transfer) notification
    Clearnode->>Bob: balance_update notification
...
- **Both parties** receive `balance_update` notification with updated balances
...
|-------|-------------|----------|
| **Authentication required** | Not authenticated | Complete [authentication](./authentication) |
| **Insufficient available balance** | Sender doesn't have enough funds in unified account | Check balance with `get_balance`, deposit or transfer funds |
| **Invalid destination address** | Malformed wallet address | Verify address format (0x-prefixed, 20 bytes) |
| **Invalid allocation format** | Negative amount or invalid structure | Check allocation structure matches API requirements |
| **Cannot transfer to self** | Sender and recipient are the same | Use different recipient address |
| **Asset not supported** | Asset not available on clearnode | Use `get_assets` to see supported assets |
| **Amount too small** | Below minimum transfer amount | Increase transfer amount |
...
| Parameter | Type | Required | Description | Format | Example | Notes |
|-----------|------|----------|-------------|--------|---------|-------|
| `account_id` | string | No | Filter by account ID | 0x-prefixed hex string (20 bytes) | `"0x1234567890abcdef..."` | Returns transactions for this account |
| `asset` | string | No | Filter by asset symbol | Lowercase string | `"usdc"` | Returns transactions for this asset only |
| `tx_type` | string | No | Filter by transaction type | One of: `transfer`, `deposit`, `withdrawal`, `app_deposit`, `app_withdrawal` | `"transfer"` | Returns only this type of transaction |
| `offset` | number | No | Pagination offset | Positive integer | `42` | Skip this many transactions |
| `limit` | number | No | Number of transactions to return | Positive integer | `10` | Default varies by implementation |
...
| `tx_type` | string | Transaction type (transfer/deposit/withdrawal/app_deposit/app_withdrawal) |
| `from_account` | string | Sender's wallet address |
| `from_account_tag` | string | Sender's user tag (empty if none) |
| `to_account` | string | Recipient's wallet address |
...
| Parameter | Type | Required | Description | Format | Example | Notes |
|-----------|------|----------|-------------|--------|---------|-------|
| `account_id` | string | No | Filter by account ID | 0x-prefixed hex string (20 bytes) | `"0x1234567890abcdef..."` | Returns entries for this account |
...
| `offset` | number | No | Pagination offset | Positive integer | `42` | Skip this many entries |
| `limit` | number | No | Number of entries to return | Positive integer | `10` | Default varies by implementation |
| `sort` | string | No | Sort order by created_at | `asc` or `desc` | `"desc"` | Default: `desc` |
...
| `account_type` | number | Account type (0=Unified, 1=App Session, 2=Channel Escrow) |
| `asset` | string | Asset symbol |
