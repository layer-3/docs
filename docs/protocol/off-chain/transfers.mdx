...
Transfer methods enable instant, off-chain fund movement between users connected to the same <Tooltip content={tooltipDefinitions.clearnode}>clearnode</Tooltip>.
...
Transfers funds from the authenticated user's <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip> to another user's <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip> within the Yellow Network. This is a purely off-chain operation executed as a database transaction on <Tooltip content={tooltipDefinitions.clearnode}>the clearnode</Tooltip>, resulting in instant settlement with zero gas fees. The transfer updates internal ledger entries using double-entry bookkeeping principles and creates a transaction record for both parties. Both sender and recipient must be connected to the same <Tooltip content={tooltipDefinitions.clearnode}>clearnode</Tooltip> (have open channels). The security guarantee comes from the underlying on-chain channels that back the <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip>.

### When to Use

When sending funds to another Yellow Network user who is connected to the same <Tooltip content={tooltipDefinitions.clearnode}>clearnode</Tooltip>. Common use cases include peer-to-peer payments, merchant payments, tipping, and rapid fund redistribution between users.

### Prerequisites

- Sender must be [authenticated](./authentication)
- Sender must have sufficient available balance (not locked in <Tooltip content={tooltipDefinitions.appChannel}>app sessions</Tooltip>)
- Recipient must have at least one open <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> with <Tooltip content={tooltipDefinitions.clearnode}>the clearnode</Tooltip> (be connected)
...
- Verifies recipient exists (has open <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip>)
...
The <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip> aggregates funds from all chains, providing a seamless cross-chain experience.
...
Users don't need to know or care which chain their funds are on. Clearnode manages the underlying <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> allocations internally. When channels close, funds are distributed according to the final <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip> state.
:::

### Balance States

<Tooltip content={tooltipDefinitions.unifiedBalance}>Unified balance</Tooltip> has three components:

1. **Total**: Sum of available + locked
2. **Available**: Funds that can be transferred or withdrawn
3. **Locked**: Funds currently in <Tooltip content={tooltipDefinitions.appChannel}>app sessions</Tooltip>
...
- When <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> closes, final on-chain distribution reflects all transfers
...
**Cannot Steal Funds**:
- <Tooltip content={tooltipDefinitions.clearnode}>A clearnode</Tooltip> cannot steal funds (would be evident when <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> closes)
- Users can always challenge-close with last signed <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip>

:::caution Security Guarantee
At every stage, funds remain cryptographically secured by the underlying payment channels. Users can always recover their funds according to the latest valid signed <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip>, even if <Tooltip content={tooltipDefinitions.clearnode}>the clearnode</Tooltip> becomes unresponsive.
:::

### What If a Clearnode Disappears?

**Recovery Process**:
- Users can challenge-close their <Tooltip content={tooltipDefinitions.channel}>channel</Tooltip> with last known <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip>
- Funds are recovered according to last signed <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip>
- Worst case: lose value of un-checkpointed transfers

**Best Practice**:
- Periodically checkpoint important <Tooltip content={tooltipDefinitions.channelState}>states</Tooltip> on-chain for large balances
...
| **Amount exceeds available** | 1002 | Exceeds available (not total) balance | Reduce amount or close <Tooltip content={tooltipDefinitions.appChannel}>app sessions</Tooltip> |
