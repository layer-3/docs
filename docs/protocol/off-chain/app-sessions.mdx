...
| **v0.2 parameters rejected** | v0.2 session receives `intent` or `version` | Omit both; only OPERATE-style redistribution is allowed |
...
| **Version mismatch** | v0.4: `version` not equal to current+1 | Fetch latest session (`get_app_sessions`) and retry with next version |
