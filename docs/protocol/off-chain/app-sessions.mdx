...
When an <Tooltip content={tooltipDefinitions.appChannel}>app session</Tooltip> is created, funds are locked from the <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip>:
...
Locked funds **cannot be transferred or withdrawn** until the <Tooltip content={tooltipDefinitions.appChannel}>app session</Tooltip> closes. Only available balance can be used for [transfers](./transfers) or new <Tooltip content={tooltipDefinitions.appChannel}>app sessions</Tooltip>.
:::
...
Every <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip> update MUST increment version sequentially:
...
- Client should fetch current <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip> and retry

:::warning Version Conflict Prevention
Version management prevents concurrent update conflicts and ensures total ordering of <Tooltip content={tooltipDefinitions.channelState}>states</Tooltip>. This guarantees all <Tooltip content={tooltipDefinitions.participant}>participants</Tooltip> see the same sequence of <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip> transitions.
:::
...
3. **Must Account for All Participants**:
   - Every <Tooltip content={tooltipDefinitions.participant}>participant</Tooltip> in definition should have an entry (even if zero)
   - Omitted <Tooltip content={tooltipDefinitions.participant}>participants</Tooltip> are treated as receiving zero
...
**Notifications**:
- <Tooltip content={tooltipDefinitions.participant}>Participants</Tooltip> are notified via WebSocket of <Tooltip content={tooltipDefinitions.channelState}>state</Tooltip> changes
