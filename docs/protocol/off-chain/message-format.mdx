...
When an error occurs, <Tooltip content="A virtual ledger layer that provides a unified ledger (through Nitro RPC) and coordinates state channels (through Nitrolite), providing chain abstraction for developers and users">the clearnode</Tooltip> sends an error response with method set to `"error"`.
...
Every RPC message (request or response) has a **payload hash** that is computed and signed. The payload is everything inside the `req` or `res` field.

### What is the Payload

- **For requests**: The `req` array: `[requestId, method, params, timestamp]`
- **For responses**: The `res` array: `[requestId, method, result, timestamp]`
...
Where `payload` is the `req` or `res` array.

### JSON Encoding Requirements

The JSON encoding MUST contain **spaces only inside string values**. No spaces between structural elements (arrays, objects, commas, colons).

**Correct encoding** for hashing:
```json
[1,"transfer",{"amount":"100","asset":"usdc","destination":"0x..."},1699123456]
```

**Incorrect encoding** (would produce different hash):
```json
[  1,  "transfer",  { "amount": "100", "asset": "usdc" },  1699123456  ]
```

:::caution Encoding Consistency
The JSON encoding logic (no spaces except in string values) may be reconsidered in the next version of the protocol. For now, all implementations MUST follow this exact encoding to ensure signature compatibility.
:::
...
**Payload Hash**:
```javascript
payloadHash = keccak256(JSON.stringify([
  42,
  "create_app_session",
...
This hash is then signed by the sender (client's session key for requests, clearnode for responses).
