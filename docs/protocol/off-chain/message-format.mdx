...
- **Type**: JSON object
- **Content**: Varies by method (see method documentation)
...
- **Generation**: Server-provided (client sends 0, server fills it)
...
The response payload, mirroring the request structure:
- Same **requestId** (to correlate with request)
- Same **method** (confirms which method responded)
- **result** instead of params (method-specific response data)
- Updated **timestamp** (server response time)
