...
- User must be [authenticated](./authentication) (<Tooltip content={tooltipDefinitions.sessionKey}>session key</Tooltip> registered)
...
| `session_key` | string (wallet address) | No | Session key address to authorize for this channel | Authenticated session key | `"0x9876543210fedcba9876543210fedcba98765432"` | Optional parameter to specify which session key can control the channel |
...
| `server_signature` | string | The clearnode's signature over the channel and state (0x-prefixed hex, 65 bytes) | — |
| `channel_id` | string | Computed channel identifier (0x-prefixed hex, 32 bytes)<br/>Computed as: keccak256(abi.encode(channel))<br/>Client should compute this independently to verify | — |
...
| Field | Type | Description | Notes |
|-------|------|-------------|-------|
| `intent` | StateIntent | State purpose indicator | For creation: 1 (INITIALIZE) |
| `version` | uint64 | State sequence number | For creation: 0 |
| `state_data` | string | Application-specific state data (hex string) | For creation: "0x" (empty) |
| `allocations` | StateAllocation[] | Fund allocations for each participant | Order matches participants array<br/>See [StateAllocation](#stateallocation) below<br/>For creation: both participants start with "0" |
...
3. **Sign State with Your Key**
   - Sign `stateHash` with your <Tooltip content={tooltipDefinitions.participant}>participant</Tooltip> key
...
| **Unsupported chain** | chain_id not supported | Use `get_config` to see supported chains |
| **Unsupported token** | Token not in asset config | Use `get_assets` to see supported tokens |
| **Invalid chain_id format** | Not a valid uint32 | Provide numeric chain ID |
| **Invalid token address** | Malformed wallet address | Verify token address format |
| **Channel already exists** | An open channel with clearnode already exists | Close existing channel first or use that channel |
| **Clearnode unavailable** | Clearnode at capacity | Retry later or contact support |
...
- The <Tooltip content={tooltipDefinitions.stateHash}>stateHash</Tooltip> should be verified: `keccak256(abi.encode(channelId, state.intent, state.version, state.data, state.allocations))`
...
| `funds_destination` | string (wallet address) | Yes | Address where your share of channel funds should be sent | - | `"0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb"` | Typically your wallet address; can also be used to send to custody ledger |

### Response

| Parameter | Type | Description | Example | Notes |
|-----------|------|-------------|---------|-------|
| `channel_id` | string | Channel identifier | `"0xabcdef1234..."` | For reference |
| `state` | State | Final state with FINALIZE intent<br/><br/>**Structure**:<br/>• `intent` - 3 (FINALIZE)<br/>• `version` - Incremented from current<br/>• `state_data` - Application-specific data<br/>• `allocations` - Final distribution of funds | See [State structure](#state-structure) | Clearnode determines final allocations based on unified balance |
| `server_signature` | string | The clearnode's signature over the final state | `"0xabcdef987654..."` | 0x-prefixed hex string (65 bytes) |
...
| **Channel not found** | Invalid channel_id | Verify channel_id from get_channels |
| **Channel not open** | Channel in wrong status | Check status; may already be closed or resizing |
| **Channel already closed** | Channel was closed | No action needed |
| **User doesn't own channel** | Not a participant | Verify you're using correct account |
| **Invalid funds_destination** | Malformed wallet address | Verify address format |
...
- <Tooltip content={tooltipDefinitions.channel}>Channel</Tooltip> must exist and be in ACTIVE status
- User must be authenticated
- For **positive delta** (deposit): Funds must be available in custody ledger
...
| `allocate_amount` | string (decimal) | No (mutually exclusive with resize_amount) | Amount to move from unified balance to channel before resize | - | `"50.0"` | Decimal string in human-readable format<br/>Used for multi-chain rebalancing |
| `resize_amount` | string (decimal) | No (mutually exclusive with allocate_amount) | Delta amount to deposit from custody ledger (positive) or withdraw to custody ledger (negative) | - | `"75.0"` (deposit) or `"-100.0"` (withdraw) | Decimal string; can be negative<br/>Positive: funds must be available in custody ledger<br/>Negative: channel must have sufficient allocated funds |
| `funds_destination` | string (wallet address) | Yes | Destination address for user's allocation in the resize state | - | `"0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb"` | 0x-prefixed hex string (20 bytes)<br/>Typically the user's wallet address |

:::note Allocate vs Resize
- **`allocate_amount`**: Moves funds from your unified balance on Clearnode to the channel
- **`resize_amount`**: Deposits from or withdraws to the custody ledger on the smart contract
- These parameters are **mutually exclusive** - provide only one per request
:::
...
| `state` | State | Resize state to be submitted on-chain<br/><br/>**Structure**:<br/>• `intent` - 2 (RESIZE)<br/>• `version` - Incremented from current (e.g., if current is 5, this will be 6)<br/>• `state_data` - ABI-encoded `int256[]` delta amounts (per participant)<br/>&nbsp;&nbsp;Example: For +50 USDC by user: `[50000000, 0]`<br/>&nbsp;&nbsp;Example: For -30 USDC by user: `[-30000000, 0]`<br/>• `allocations` - FINAL allocation state after resize (absolute amounts, not deltas) | See [State structure](#state-structure) | Clearnode prepares deltas and final allocations |
| `server_signature` | string | The clearnode's signature over the resize state | `"0x9876fedcba..."` | 0x-prefixed hex string (65 bytes) |
...
3. **Ensure sufficient funds in custody ledger (if depositing)**
   - For positive `resize_amount`, the requested amount must already be available in your custody ledger
   - Funds are moved from custody ledger to the channel
   - No token approval needed at this stage
...
| Channel not found | Invalid channel_id | Verify <Tooltip content={tooltipDefinitions.channelId}>channel ID</Tooltip> with `get_channels` |
| Channel not in ACTIVE status | Channel closed or in dispute | Cannot resize non-active <Tooltip content={tooltipDefinitions.channel}>channels</Tooltip> |
| Insufficient balance | Not enough available in custody ledger for deposit | Check `get_balance` for available amount in custody ledger |
...
**On-Chain**:
- User must have 75 USDC already available in custody ledger
- Calls `resize()` with positive delta: `[+75000000, 0]`
- Custody locks additional 75 USDC from custody ledger to channel
- Channel allocations updated: 5 USDC → 80 USDC
