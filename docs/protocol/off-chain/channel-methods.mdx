...
6. **Monitor for Channel Opening**
   - Listen for `Opened` event (emitted when <Tooltip content={tooltipDefinitions.clearnode}>the clearnode</Tooltip> joins)
   - Or poll `get_channels` until status is "active"

7. **Channel Active**
   - Channel appears in <Tooltip content={tooltipDefinitions.unifiedBalance}>unified balance</Tooltip>
...
- After closing, funds return to "available" balance in <Tooltip content={tooltipDefinitions.custodyContract}>Custody contract</Tooltip>
