...
description: "Nitrolite state structure, channel ledgers, off-chain and enforcement representations, intent mapping, and consistency rules."
---
...
![State ledger advancement](/img/protocol/state-ledger-advancement.png)
...
| On-chain intent | Transition literal | Meaning |
| --- | --- | --- |
...
| `INITIATE_ESCROW_DEPOSIT` | `mutual_lock` | Escrow deposit initiate. |
| `FINALIZE_ESCROW_DEPOSIT` | `escrow_deposit` | Escrow deposit finalize or complete. |
| `INITIATE_ESCROW_WITHDRAWAL` | `escrow_lock` | Escrow withdrawal initiate. |
| `FINALIZE_ESCROW_WITHDRAWAL` | `escrow_withdraw` | Escrow withdrawal finalize or complete. |
| `INITIATE_MIGRATION` | `migrate` | Home-chain migration initiate. |
