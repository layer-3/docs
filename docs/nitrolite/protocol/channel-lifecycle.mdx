...
description: "Channel definition, identifiers, lifecycle actions, signing categories, transition families, and enforcement-relevant transitions."
---
...
stateDiagram-v2
  [*] --> Create
  Create --> Enforced: enforce signed state
  Create --> Close: cooperative close
  Enforced --> Enforced: higher-version state
  Enforced --> Challenge: participant dispute
  Challenge --> Enforced: higher-version response
  Challenge --> Close: challenge expires
  Enforced --> Close: final signed state
  Close --> [*]
```
...
| State enforcement | Off-chain, then optionally on-chain | The Node validates and stores a new state off-chain. Depending on transition type or participant initiative, the state MAY also be submitted to the blockchain layer. |
...
| Home Deposit | Records an asset deposit from the home chain. `AccountId` MUST reference the home channel identifier. Requires on-chain enforcement to lock deposited assets. |
| Home Withdrawal | Records an asset withdrawal to the home chain. `AccountId` MUST reference the home channel identifier. Requires on-chain enforcement to release withdrawn assets. |
...
| Transition | Intent | Enforcement behavior |
| --- | --- | --- |
