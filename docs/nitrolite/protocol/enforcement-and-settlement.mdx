...
2. The channel MUST NOT already be `challenged` or `closed`.
...
5. The channel status becomes `challenged`, and challenge expiry is set to current time plus challenge duration.
...
Escrow and migration transitions are part of the protocol model, but their public app-builder workflow is not documented yet. Treat the enforcement details as protocol reference material, not as instructions for constructing escrow or migration states by hand.
