...
The current schema names the two escrow-withdrawal transitions as:

| Transition | Meaning |
| --- | --- |
| `escrow_lock` | Escrow withdrawal initiate. Funds are prepared for the cross-chain withdrawal path. |
| `escrow_withdraw` | Escrow withdrawal finalize. The withdrawal is completed toward the non-home-chain destination. |

These names are included so readers can recognize them in protocol and API reference material. They are not instructions to assemble escrow withdrawal states by hand.
