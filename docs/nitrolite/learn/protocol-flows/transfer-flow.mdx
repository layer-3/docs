...
**Serialized operations**: Nitronode does not accept a transfer while another state transition is still pending for the same user and asset. Escrow and migration transitions are not documented as builder-facing workflows yet, but they follow the same rule: finish or acknowledge the pending state before submitting a normal transfer.
:::
