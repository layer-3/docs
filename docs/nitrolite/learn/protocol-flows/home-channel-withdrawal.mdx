...
This is a **single-chain operation** that updates the state and enforces it on-chain to release funds.
...
| **Node** | The Nitronode that validates and stores state transitions |
...
:::note
The "no ongoing operation" requirement applies to ALL operations except the RPC operation names `finalize_escrow_deposit`, `finalize_escrow_withdrawal`, and `finalize_migration`. Those names describe protocol/RPC surfaces, not current app-builder escrow or migration how-to steps.
:::
...
sequenceDiagram
    ...
    Client->>Node: Query latest channel and balance
    Node-->>Client: Returns latest channel state and balance
    Client-->>User: Returns success
```

Nitronode listens for blockchain events internally and updates its stored view:
...
2. **On-chain enforcement** -- ChannelHub validates all signatures.
...
| **Enforcement revert** | On-chain validation failure | Check state validity |
