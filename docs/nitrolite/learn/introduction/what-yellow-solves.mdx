...
State channels let applications execute many signed off-chain operations between occasional on-chain enforcement actions.
...
3. **Move across chains through escrow where supported.** The protocol defines two-phase escrow states that coordinate the home ledger and non-home ledger, but public builder workflows for escrow are not documented yet.
4. **Settle when needed.** Any party may enforce, challenge, or close with an enforceable mutually signed state.
...
- **DeFi:** High-frequency trading, prediction markets, and escrow-backed flows once the public builder workflow is available.
