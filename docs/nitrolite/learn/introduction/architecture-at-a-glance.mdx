...
The v1 RPC currently exposes channel status as `void`, `open`, `challenged`, and `closed`. After a challenge expires, a separate close call can move the channel to `closed`; there is no separate `closing` status in the RPC schema.
