...
After publication, connect the MCP server to your MCP-compatible client. Choose the setup that matches your tool.
...
Any MCP-compatible client that supports stdio servers can launch the package with the same `npx -y @yellow-org/sdk-mcp@^1` command after publication.

Then use the MCP as a second review pass after the codemod:
...
MCP assistance does not replace the manual checklist below. It gives your coding tool the SDK, compat, Go SDK, protocol, examples, and migration context it needs to review the migration with fewer stale assumptions.
