...
The Yellow SDK MCP server is planned for npm as `@yellow-org/sdk-mcp`, but it is not published yet. After it is available, connect it to an MCP-compatible coding tool and use it as a migration assistant alongside the codemod. Setup examples are available for Claude Code, Claude Desktop, Codex, Cursor, and VS Code in the [migration overview](./migration-overview#mcp-assisted-migration).

```bash
# Coming soon: client configs launch this only after @yellow-org/sdk-mcp is published
npx -y @yellow-org/sdk-mcp@^1
```

Then ask your assistant to run the `migrate-from-v053` prompt, check uncertain symbols with `validate_import`, map old RPC helper chains with `get_rpc_method`, and verify native SDK replacements with `lookup_method`, `lookup_type`, or `search_api`.
:::
