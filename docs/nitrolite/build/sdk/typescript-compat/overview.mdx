...
The Yellow SDK MCP server is planned for npm as `@yellow-org/sdk-mcp`, but it is not published yet. After it is available, connect it to an MCP-compatible coding tool and use it as a migration assistant alongside the codemod:

```bash
# Coming soon: run this only after @yellow-org/sdk-mcp is published
