...
`EventPoller` does not renew auth tokens, persist state, or replace your app cache. It gives migrated apps a single start/stop lifecycle and one error path while they move away from pushed WebSocket events.
