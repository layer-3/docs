...
Security-token calls are for node-operator or administrative tooling, not the normal application path.

```go
if _, err := client.ApproveSecurityToken(ctx, chainID, decimal.NewFromFloat(100)); err != nil {
    return err
}

if _, err := client.EscrowSecurityTokens(ctx, operatorWallet, chainID, decimal.NewFromFloat(100)); err != nil {
    return err
}
```
